...
<fieldset class='data-container'>
	...
	<table class="pretty-table wslogging-table">
		...
		<tbody>
			...
			<tr>
				<td><a
					href="<?php echo $this->createUrl('site/viewlogging',array('h'=>AppHelper::two_way_string_encrypt($wslogging->id)))?>"
					title="View WsLogging Deatils of <?php echo($wslogging->method_name);?>"><?php echo($wslogging->id);?>
				</a>
				</td>
				<td><?php echo($wslogging->id_site);?></td>
				<td><?php echo($wslogging->remote_address);?></td>
				<td><?php echo($wslogging->method_name);?></td>
				<td><?php echo($wslogging->api_key);?></td>
				<td><?php echo($wslogging->response_type);?></td>
				<td><?php echo($wslogging->handler_name);?></td>
				<td><?php echo($wslogging->request_type);?></td>
				<td><?php echo($wslogging->status);?></td>
				<td><?php echo($wslogging->date_and_time);?></td>
			</tr>
			<?php } ?>
		</tbody>
	</table>
