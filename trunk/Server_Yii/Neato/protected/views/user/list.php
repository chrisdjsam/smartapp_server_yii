...
?>
	...
	<p class="list_details">
		...
		You can also select a user and click on delete button to delete a user.
		<br />
		Please note that deleting a user would also delete the user-robot associations for that specific user.
		<br />
		<?php }?>
		<?php }?>
	</p>
	...
	<?php }?>
	<form action="<?php echo $this->createUrl('user/delete') ?>" method="POST" id="userList">
		<?php if($userRole != '2'){?>
		<?php if(!$is_wp_enabled){ ?>
		<div class="action-button-container">
			<a href="<?php echo $this->createUrl('user/add')?>" title="Add user" class="neato-button">Add</a>
			<input type='button' value='Delete' id='deleteSelected' class='deleteSelected neato-button' title="Delete user" />
		</div>
		<?php }?>
		<?php }?>
