...
?>
	...
	<p class="list_details">
		...
		<?php }?>
		<?php if($userRole == '2'){?>
		You can search a specific robot by typing in serial number and then clicking on search button.
		<br />
		<?php }?>
	...
	<?php if($userRole == '2'){?>
		<div class="action-button-container support-delete-div">
			<input type='button' value='Delete' id='deleteSelected' class='deleteSelected neato-button' title="Delete robot" />
		</div>
	...
	<form action="<?php echo $this->createUrl('api/robot/deleteRobot') ?>" method="POST" id="robotsList">
		...
		<div class="action-button-container">
			<a href="<?php echo $this->createUrl('robot/add')?>" title="Add robot" class="neato-button">Add</a>
		</div>
		<?php }?>
